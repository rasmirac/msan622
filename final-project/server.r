...
scale_year <- function(mini, maxi) {
  return(
    scale_x_date(
      ...
      breaks = seq(as.Date(mini, '%Y'), by='4 years', length=17),
      expand = c(0, 0), 
...
linePlot <- function(localFrame, expandGroup, range) {
  ...
  if (expandGroup != 'None'){
    ...
    line_plot <- ggplot(localFrame, 
      ...
      ggtitle('UK Household Average Food Purchases per Person, 1974-2012')
    ...
    line_plot <- ggplot(localFrame, aes(x = as.Date(variable, '%Y'), 
      ...
      scale_color_brewer(palette = 'Dark2', name = ' ') + 
      ggtitle('UK Household Average Purchases per Person, 1974-2012') + 
      theme(
        legend.direction = "horizontal",
        legend.position = c(1, 1),
        legend.justification = c(1, 1),
...
heatPlot <- function(localFrame, group, facet) {  
  if (facet == F){
    ...
    my_heat_map <- ggplot(localFrame, aes(x = as.Date(variable, '%Y'), y = country)) + 
      ...
      theme_heatmap() + 
      ggtitle(paste('Average Units of', group,'Purchased per Person by Country, 2001 - 2012', ' ')) + 
      scale_x_date(
        name = "Year",
...
areaPlot <- function(localFrame, showCountry) { 
  ...
  my_area_plot <- ggplot(subset(localFrame, as.Date(as.character(variable), '%Y') >= as.Date('1992', '%Y')), 
    ...
    theme(
      ...
      panel.background = element_rect(fill = NA), 
      panel.grid.major = element_line(colour = 'gray90')) + 
    ggtitle('Average Household Food Purchases per Person in UK, 1974 - 2012') + 
